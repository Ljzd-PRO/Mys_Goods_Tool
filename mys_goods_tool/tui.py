...
class GoodsWidget(ExchangePlan.BasePlanAdding):
    ...
    class GoodsDictValue:
        ...
        def __init__(self,
                     game_info: GameInfo,
                     button_select: Optional[GoodsWidget.GameButton] = None,
            ...
            """商品分区数据"""
            self.button_select = button_select or GoodsWidget.GameButton(
                "💾 确定",
                id=f"button-goods-select-{game_info.id}",
                disabled=True,
                game=game_info)
            """选择商品的按钮"""
            self.option_list = OptionList(GoodsWidget.empty_option_list)
            """商品的选项列表"""
