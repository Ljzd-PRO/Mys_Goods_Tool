...
from typing import NamedTuple, Tuple, Optional, Set, List, Dict, Any, Callable
...
from mys_goods_tool.api import create_mobile_captcha, create_mmt, get_login_ticket_by_captcha, \
    ...
    get_good_list, get_game_list, get_address
...
from mys_goods_tool.data_model import GeetestResult, MmtData, GetCookieStatus, Good, GameInfo, Address
...
class ExchangePlan(Container):
    ...
    def _on_compose(self) -> None:
        ExchangePlan.loop = asyncio.get_event_loop()


class AccountWidget(ExchangePlan.BasePlanAdding):
    """
    选择账号 - 界面
    """
    ...
    def _on_button_pressed(self, event: ControllableButton.Pressed) -> None:
        if event.button.id == "button-account-select":
            ...
            task = ExchangePlan.loop.create_task(AddressWidget.update_address(self.app.notice))
            task.add_done_callback(ExchangePlan.loop_tasks.discard)
...
class GoodsWidget(ExchangePlan.BasePlanAdding):
    """
    选择商品 - 界面
    """
    DEFAULT_CSS = """
